...
namespace graphics {
	...
	GLuint Shader::load()
	{
		...
		if (result == GL_FALSE)
		{
			...
			std::cout << "Could not compile vertex shader! " << std::endl << &error[0] << std::endl;;
		...
		if (result == GL_FALSE)
		{
			...
			std::cout << "Could not compile fragment shader! " << std::endl << &error[0] << std::endl;;
	...
	void Shader::setUniform1i(const char *name, GLuint value)
	{
